#!/usr/bin/env python3
"""
task56_hydrogen_peroxide.py -- iteration 56. Testing the validated forging
+ ancilla-parity + conditioned-PEC/GPi2 + joint-Schmidt-frame pipeline on
hydrogen peroxide (H2O2) -- a real reactive-oxygen-species molecule
directly implicated in oxidative DNA damage, a well-established mechanism
driving mutation and carcinogenesis (H2O2 -> 2 OH* via O-O homolysis is
the textbook first step).

GEOMETRY: real experimental gauche equilibrium structure -- r(O-O)=1.475
Ang, r(O-H)=0.965 Ang, angle(H-O-O)=94.8 deg, dihedral(H-O-O-H)=113.7 deg.
Verified by direct measurement of the constructed Cartesian coordinates
(O-O dist=1.475000, both O-H dists=0.965000, exact).

WHY AN ACTIVE SPACE: H2O2's full space is 12 spatial orbitals (24 qubits),
infeasible for exact diagonalization for the same reason as formaldehyde
(task55). A (4 electrons, 4 orbitals) frozen-core active space (freeze the
7 lowest canonical RHF orbitals -- both O 1s cores plus the O-H sigma
bonding framework -- keep the next 4 as active, covering the O-O sigma/
sigma* and oxygen lone-pair frontier region that governs O-O bond
homolysis) targets exactly the chemistry relevant to H2O2's role in
oxidative damage, not an arbitrary truncation.

CROSS-CHECK: the active-space Hamiltonian built here (dense eigh) matches
molecules_real.ground_state's own independent sparse-eigsh computation of
the SAME active space EXACTLY (diff 0.0000 kcal/mol): HF=-148.742128 Ha,
exact(active)=-148.749024 Ha.

REAL FINDING: like formaldehyde, this (4e,4o) active space's register (4
qubits, weight-2) is IDENTICAL in shape to H4's own, and its Schmidt rank
again saturates at EXACTLY K=6 -- H4's own hand-optimized ansatz
(fixed_ansatz.py) is reused directly, refit to H2O2's own Schmidt vectors.

HONESTY, stated plainly: this computes H2O2's real electronic ground-state
energy in a real, disclosed active space. It says nothing about how H2O2
actually behaves biologically inside a cell, its reaction kinetics, or any
therapeutic property -- this is a test of whether the validated
measurement METHOD generalizes to a second, independently chosen real
molecule tied to cancer biology, nothing more.

Run:
    PYTHONHASHSEED=0 python vqe/task56_hydrogen_peroxide.py            # full build+verify+submit+analyze
    PYTHONHASHSEED=0 python vqe/task56_hydrogen_peroxide.py --analyze  # analyze already-submitted data only
    python vqe/task56_hydrogen_peroxide.py --classical-only            # setup+verify only, no network calls
"""
import os
import sys
import json
...
from fixed_ansatz import build_ansatz, fit_angles
...
# Real experimental gauche geometry (Cartesian, built from internal
# coordinates and verified by direct bond-length measurement).
r_OO, r_OH = 1.475, 0.965
ang_HOO = np.radians(94.8)
dih = np.radians(113.7)
O1 = np.array([0.0, 0.0, 0.0])
O2 = np.array([r_OO, 0.0, 0.0])
H1 = O1 + r_OH * np.array([-np.cos(ang_HOO), np.sin(ang_HOO), 0.0])
y2 = r_OH * np.sin(ang_HOO) * np.cos(dih)
z2 = r_OH * np.sin(ang_HOO) * np.sin(dih)
x2 = r_OO + r_OH * np.cos(np.pi - ang_HOO)
H2 = np.array([x2, y2, z2])
GEOM = [("O", tuple(O1)), ("O", tuple(O2)), ("H", tuple(H1)), ("H", tuple(H2))]
...
TRUSTED_ACTIVE_EXACT_HA = -148.749024
TRUSTED_HF_HA = -148.742128
OUT_PATH = os.path.join(os.path.dirname(__file__), "task56_hydrogen_peroxide_results.json")
...
def build_active_space_qop():
    ...
    enuc_effective = enuc + e_core
    return qop_bare, qop_pen, enuc_effective, ehf


def setup_h2o2_fragment(K):
...
def setup_and_verify():
    p = setup_h2o2_fragment(K)
    non_id_labels = sorted(l for l in p["alpha_labels"] if l != p["identity_label"])
    diag, plus, kept = kept_slots_for_K(K)
    print(f"  H2O2, (4e,4o) active space: n_qubits={p['n_qubits']}, K={K} (exact rank)")
    ...
    print(f"  classical K={K} truncation error vs exact: {p['truncation_err_vs_exact_kcal']:.6f} kcal/mol")
    ...
    assert max_leakage < 1e-9, "H2O2's active-space targets are not confined to weight-2 -- STOP"

    groups = group_labels_qubit_wise(non_id_labels)
    print(f"  {len(groups)} qubit-wise-commuting groups")

    print(f"  Fitting H4's own 5-parameter ansatz to H2O2's {len(kept)} targets...")
    fixed_solutions = {}
    worst = 0.0
    for name in kept:
    ...
    assert worst < 1e-10, "H4's ansatz could not reach H2O2's own targets to machine precision -- STOP"
...
def analyze(p=None, non_id_labels=None, kept=None, groups=None, fixed_solutions=None):
    ...
    print(f"\n  H2O2 (4e,4o active space) exact energy: {p['exact_energy']:.6f} Ha")
    results = {}
    for backend_name in BACKENDS:
